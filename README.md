...
โปรเจกต์นี้ถูก Deploy เป็น **Web Service** เดียวบน Render เพื่อให้บริการทั้ง API และ Static Files

### Deployment Checklist

| ขั้นตอน | สถานะ | รายละเอียด |
| :--- | :--- | :--- |
| **1. Commit & Push** | ✅ | โค้ดทั้งหมดต้องอยู่บน GitHub (รวมถึง `Assignment_2`) |
| **2. Update `app.js`** | ⚠️ | **ต้องแก้ไข** `API_BASE_URL` ใน `app.js` เป็น Public URL ของ Render Service ก่อน Push |
| **3. Configuration** | ✅ | ตั้งค่า **Build Command** เป็น `npm install` และ **Start Command** เป็น `npm start` |
| **4. Environment Vars** | ✅ | **เพิ่มตัวแปรทั้งหมด** จาก `.env` (ยกเว้น `PORT`) ลงใน Render Dashboard |
