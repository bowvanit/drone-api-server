...
const LOGS_PER_PAGE = 12; // กำหนดให้แสดงผล 12 รายการต่อหน้า
let currentPage = 1;
...
function renderPagination(hasNext) {
    ...
    if (currentPage > 1) {
        html += `<button id="prev-page-btn" class="px-4 py-2 mx-1 bg-pink-500 text-white font-medium rounded-lg shadow-md hover:bg-pink-600 transition duration-300 transform hover:scale-105" onclick="changePage(-1)">
    ...
    // **** แก้ไข: ปุ่ม Next แสดงผลเสมอ (ยกเว้นหน้า 1 ที่ไม่มีข้อมูลเลย) ****
    // เพื่อให้ผู้ใช้กดไปหน้าถัดไปได้ทันที
    if (currentPage === 1 || hasNext) { 
        html += `<button id="next-page-btn" class="px-4 py-2 mx-1 bg-pink-500 text-white font-medium rounded-lg shadow-md hover:bg-pink-600 transition duration-300 transform hover:scale-105" onclick="changePage(1)">
...
/**
 * เปลี่ยนหน้าสำหรับ Logs และเรียกข้อมูลใหม่
 * @param {number} delta 
 */
